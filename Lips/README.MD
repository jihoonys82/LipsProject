...
### .jsp
  + jsp 에서 유저정보 호출하기
    ...
    =>
    <pre><code><sec:authentication property="principal.userId"/></pre></code>

    3. 사용자의 권한을 확인할 경우
    <pre><code>${sessionScope.user.userLevel eq 2} =>  <sec:authorize access="hasAuthority('USER')"></pre></code>
