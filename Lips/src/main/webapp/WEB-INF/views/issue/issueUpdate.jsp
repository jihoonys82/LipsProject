		...
		<div class="body">
			<form action="/issue/create" method="post" enctype="multipart/form-data" onsubmit="customValue();">
				...
				<div class="issue-form-row">
					...
					<input name="projectId" id="projectId" class="input issue-form-input" readonly="readonly" value="[${issue.projectId }]  ${projectName }"/>
				</div>
				<div class="issue-form-row">
					<label for="issueTitle" class="issue-form-label">제목</label>
					<input type="text" name="issueTitle" id="issueTitle" class="input issue-form-input" readonly="readonly" value="${issue.issueTitle }"/>
				</div>
				<div class="issue-form-row">
				...
				<div class="issue-form-row">
					...
					<input type="date" name="expectedEndDate" id="expectedEndDate" class=" input issue-form-input" value="<fmt:formatDate value='${issue.expectedEndDate }' pattern='yyyy-MM-dd' />"/>
				</div>
				<div id="assign" class="issue-form-row">
					<label for="assignee" class="issue-form-label">담당자</label>
					<input type="text" name="assignee" id="assignee" class="input issue-form-input" value="${issue.assignee }"/>
					<c:if test='${issue.assignee ne userId }'>
...
<!-- 					<button type="button" class="btn" onclick="setStageModal.show();">단계 설정</button> -->
				...
				<div class="issue-form-row">
					...
					<textarea name="issueContent" id="issueContent" class="input issue-form-input" rows="10" >${issue.issueContent }</textarea>
				</div>
				<div class="issue-form-row">
			...
			<div class="issue-form-row" id="customDiv">
				<div id="issueCustom1">
					...
					<p style="text-align:center" id="customNotice">컬럼타입, 컬럼명을 지정하지 않을 경우 해당컬럼의 데이터가 저장되지 않습니다.<p>
				 ...
				 $(document).ready(function(){
					...
					for( var customIndex = 0;customIndex<customList.length;customIndex++){
						$("<div><input type='text' id='custom"+i+"type' class='input customType' style='margin-left:10px;' readonly='readonly' value='"+customList[customIndex].type+"' >"+
								"<input type='text' id='custom"+i+"Name' class='input customName' style='margin-left:10px;' value='"+customList[customIndex].name+"' >"+
								"<input type='text' id='custom"+i+"value' class='input customInput' style='margin-left:10px;' value='"+customList[customIndex].value+"' ></div>"					 
							 ).prependTo($('#issueCustom1'));
						i++;
					}
					$.ajax({
						type:"post"
						, url: "/issue/getStageAssets"
						, async : false
						, data: { "issuePresetId" : ${issue.stagePresetId } }
						, dataType: "json"
						, beforeSend: function () {
							$("#selectedStage").empty();
						}
